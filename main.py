...
def generate_data_cluster(num_points=100, sample_balance=10, k=2, dims=3):
    # generate n-D data clusters
    sample_prob = np.random.dirichlet(np.ones(k)*sample_balance, size=1)[0]
    means = np.multiply(np.random.rand(k, dims), np.random.randint(1, 4, dims))
...
def cluster_data_example():
    ...
    data = generate_data_cluster(num_points=500, sample_balance=100)
    x_data = data[0].T
